---
title: "An Analysis of Climate Sentiment and Term Frequency Across the USA"
...
cat(paste("Total Words: ", as.character(length(BM_Words$word))))
...
cat(paste("Total Words: ", as.character(length(CH_Words$word))))
# removing any stop words.
...
cat(paste("Total Words: ", as.character(length(SFNM_Words$word))))
# Removing any stop words.
...
cat(paste("Total Words: ", as.character(length(TBT_Words$word))))
...
cat(paste("Total Words: ", as.character(length(LA_Words$word))))
...
cat(paste("Total Words: ", as.character(length(Seattle_Words$word))))
...
region_words <- region_words[nchar(region_words$word) > 2, ]

print.data.frame(head(region_words, 25))

```

#### We unsuprisingly see a lot of location-based terms appearing, such as town names.{.unlisted .unnumbered}
...
### We don't get a ton of information from this project that would influence policy suggestions, but one approach could be to gauge the specific climate-related needs of each region by looking at their sentiment and highest tf-idf terms. For example, the word "air" is used more in the LA articles than any other source, suggesting that air quality could be a bigger climate focus in that region.{.unlisted .unnumbered}
